...
bool Vehicle::get_vehicle_ahead(vector<vector<double>> sensor_fusion, Vehicle & rVehicle, double futurePredictionTime) {
  /*
  ...
  */
  ...
  for (int i =0; i < sensor_fusion.size(); i++)
  {
    ...
    {
      float predictedS = mS * mVelocity * futurePredictionTime;
      ...
      if (found_vehicle)
      {
        ...
        if( (predictedS - rVehicle.mS) < predictedS - (s + velocity * futurePredictionTime) )
        {
      ...
      // rVehicle.mYaw = 0; // Don't have but want to put  value
      rVehicle.mVelocity = velocity;
      rVehicle.mS = s + rVehicle.mVelocity * futurePredictionTime;
...
void Vehicle::Vehicle(float targetSpeed, float goalLane, float minFollowDistance, float maxLane = 12, float minLane 0, float maxAccel = 10, float timeStep = 0.02) {
  /*
  ...
  */
  ...
  mMaxLane = maxLane;
  mMinLane = minLane;
  mMaxAcceleration_mpss = maxAcceleration;
  mTimeStep = timeStep;
